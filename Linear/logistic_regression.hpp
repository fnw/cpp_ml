...
void LogisticRegression::fit(const Eigen::ArrayXXf &x, const Eigen::ArrayXXf &y,
  ...
  for (int count = 0; count < maxIter && error.abs().mean() > 0.001; ++count) {
    ...
    error = (fitX.colwise() * ((y - pred)(Eigen::indexing::all, 0)))
                .colwise()
                .sum();
